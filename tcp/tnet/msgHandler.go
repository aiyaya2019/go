...
func NewMsgHandle() *MsgHandle {
	return &MsgHandle{
		...
		TaskQueue:      make([]chan tiface.IRequest, utils.GlobalObject.MaxWorkerTaskLen),
...
func (mh *MsgHandle) AddRouter(msgId uint32, router tiface.IRouter) {
	...
	if _, ok := mh.Apis[msgId]; ok {
		//id已经注册了
		//panic() 是一个内置函数，用于引发运行时异常（panic）。当发生无法恢复的错误或异常情况时，panic() 函数可以用于中断程序的正常执行流程，并触发程序的崩溃
...
func (mh *MsgHandle) SendMsgToTaskQueue(request tiface.IRequest) {
	//1、将消息平均分配给不同的worker
	//根据客户端的ConnId来进行分配（有requestId可用requestId）
